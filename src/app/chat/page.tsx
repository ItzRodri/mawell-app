...
export default function ChatPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      content: "¡Hola Jose! Soy tu asistente de IA. ¿En qué puedo ayudarte hoy?",
      role: "assistant",
      timestamp: new Date(),
    },
  ])
    ...
    setTimeout(() => {
      ...
      switch (command.category) {
        ...
        case "action":
          ...
          break
        default:
          responseContent = "Entiendo tu solicitud. ¿Podrías darme más detalles para poder ayudarte mejor?"
      }
  ...
  const shouldShowQuickCommands = () => {
    return messages.length <= 1 || messages[messages.length - 1]?.role === "assistant"
  }
    ...
    <div className="h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex flex-col">
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-3 flex items-center gap-3 shadow-sm">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-slate-500">{isTyping ? "Escribiendo..." : "En línea"}</p>
        </div>
      </div>

      {/* Messages Area */}
      <div ref={scrollAreaRef} className="flex-1 overflow-y-auto p-4" style={{ height: "calc(100vh - 140px)" }}>
              ...
              <div className={`flex flex-col ${msg.role === "user" ? "items-end" : "items-start"}`}>
                <div
                  className={`max-w-xs sm:max-w-md lg:max-w-lg px-4 py-3 rounded-lg shadow-sm ${
                    msg.role === "user" ? "bg-blue-500 text-white" : "bg-white border border-slate-200"
                  }`}
                >
                  <p className="text-sm leading-relaxed whitespace-pre-wrap">{msg.content}</p>
                </div>

                <span className="text-xs text-slate-500 mt-1 px-1">{formatTime(msg.timestamp)}</span>
              </div>
          ...
          {shouldShowQuickCommands() && !isTyping && (
            <div className="mt-6 space-y-4">
              <div className="text-center">
                <p className="text-sm text-slate-600 font-medium mb-3">Comandos rápidos para empezar</p>
              </div>
                ...
                {quickCommands.map((command) => (
                  ...
                  >
                    <div className="w-full">
                      ...
                      <p className="text-xs text-slate-500 mt-1">{command.message}</p>
                    </div>
                  </button>
                ))}
              </div>
              ...
              <div className="text-center">
                <p className="text-xs text-slate-400">O escribe tu propia pregunta abajo</p>
              </div>
            </div>
          )}
        </div>
